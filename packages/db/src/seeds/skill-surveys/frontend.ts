...
/**
 * フロントエンドスキルアンケートのシードデータを DB に投入する（idempotent）。
 *
 * upsert 方式（onConflictDoUpdate）を全テーブルで統一使用。
 * 各テーブルの id は初回生成後不変（set に id を含めない）。
 * backend / ai-driven-development seed と同型。
 */
export async function runFrontendSkillSurveySeed(db: DB): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    const [survey] = await tx
      .insert(skillSurvey)
      .values({
        jobType: frontendSurveySeed.jobType,
        title: frontendSurveySeed.title,
      })
      .onConflictDoUpdate({
    ...
    for (const category of frontendSurveySeed.categories) {
      // 2. category をアップサート
      const [cat] = await tx
    ...
    console.log(
      `[skill-survey/frontend] categories: ${totalCategories}, questions: ${totalQuestions}, choices: ${totalChoices}`,
    );
  });
}
