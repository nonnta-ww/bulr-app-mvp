...
/**
 * バックエンドエンジニア スキルアンケートのシードデータを DB に投入する（idempotent）。
 *
...
export async function runBackendSkillSurveySeed(db: DB): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    const [survey] = await tx
      .insert(skillSurvey)
      .values({
        jobType: backendSurveySeed.jobType,
        title: backendSurveySeed.title,
      })
      .onConflictDoUpdate({
    ...
    for (const category of backendSurveySeed.categories) {
      // 2. category をアップサート
      const [cat] = await tx
    ...
    console.log(
      `[skill-survey/backend] categories: ${totalCategories}, questions: ${totalQuestions}, choices: ${totalChoices}`,
    );
  });
}
