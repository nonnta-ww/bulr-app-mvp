...
/**
 * インフラ・SRE スキルアンケートのシードデータを DB に投入する（idempotent）。
 *
 * upsert 方式（onConflictDoUpdate）を全テーブルで統一使用。
 * 各テーブルの id は初回生成後不変（set に id を含めない）。
 * backend / frontend / ai-driven-development seed と同型。
 */
export async function runInfrastructureSreSkillSurveySeed(db: DB): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    const [survey] = await tx
      .insert(skillSurvey)
      .values({
        jobType: infrastructureSreSurveySeed.jobType,
        title: infrastructureSreSurveySeed.title,
      })
      .onConflictDoUpdate({
    ...
    for (const category of infrastructureSreSurveySeed.categories) {
      // 2. category をアップサート
      const [cat] = await tx
    ...
    console.log(
      `[skill-survey/infrastructure-sre] categories: ${totalCategories}, questions: ${totalQuestions}, choices: ${totalChoices}`,
    );
  });
}
