...
> **更新 2026-05-23**: Stage 2 の構造変化は再設計され、候補者(bulr.net)・企業(bz.bulr.net)・運営(admin.bulr.net)の3アプリ構成＋`packages/{auth,ui}` 切り出しとして具体化された。Wave 別の詳細ロードマップは `docs/superpowers/specs/2026-05-23-bulr-candidate-business-split-design.md` と `roadmap.md` を参照。current-state（単一アプリ `apps/web`）の記述は各 Wave 実装時に同期更新する。以下は当初の想定（参考）。
