...
`monorepo-foundation` で `pnpm dev` がローカルで動く状態は作るが、Vercel デプロイ・Neon 接続・Resend 統合・Vercel Blob・Vercel Cron・環境変数管理の規約がない。Stage 1 のゴール（ベトナム人 20-30 + 日本人 10-20 への配信）を達成するには、本番デプロイと PR ごとの Preview 環境、音声30日自動削除の Cron が必須。

v2 移行に伴い、v1 から **OpenAI Whisper API キー（OPENAI_API_KEY）**、**Vercel Blob トークン（BLOB_READ_WRITE_TOKEN）**、**Vercel Cron 認証（CRON_SECRET）** の追加と、**vercel.json で Cron 定義** が必要。

## Current State

- `monorepo-foundation` 完了後: `apps/web` がローカル起動可能、`packages/db` に Drizzle 初期化あり、しかし DB 接続文字列なし、Vercel 未設定、Resend / Vercel Blob 未設定、Cron 未設定
- 参照プロジェクト `dishxdish-app-mvp` が `multi-env-infrastructure` spec で 4 環境構成（local / dev / preview / prod）を実装済み。bulr では Stage 1 の規模感に合わせて 2 環境構成（dev branch + production）に簡略化、加えて Vercel Blob と Cron を追加
...
- OpenAI API キーが取得され、Vercel に登録されている
- Vercel Blob ストア (`bulr-audio`) が作成され、`BLOB_READ_WRITE_TOKEN` が Vercel 環境変数に自動設定される
- `CRON_SECRET` が Vercel に登録され、Vercel Cron 認証で利用される
- `vercel.json` に Cron 定義（`/api/cron/audio-purge` を毎日 03:00 JST に実行）
...
dishxdish の `multi-env-infrastructure` spec から「Vercel + Neon ブランチ分離 + 環境変数規約」の核を抽出し、Stage 1 用に 2 環境構成（local / dev / production の 3 段階、Preview は dev DB を共有）に簡略化。加えて Vercel Blob + Vercel Cron を追加（dishxdish にはない bulr 独自）。
...
- **Vercel Blob**: 単一ストア `bulr-audio`、Vercel ダッシュボードから作成、`BLOB_READ_WRITE_TOKEN` が自動付与
- **Vercel Cron**: `vercel.json` で `/api/cron/audio-purge` を毎日定時実行、認証は `CRON_SECRET`
...
- **In**:
  ...
  - OpenAI アカウント作成 + API キー取得手順の文書化
  - Vercel Blob ストア (`bulr-audio`) 作成手順の文書化
  - `.env.example` 作成（DATABASE_URL / BETTER_AUTH_SECRET / BETTER_AUTH_URL / RESEND_API_KEY / NEXT_PUBLIC_APP_URL / ANTHROPIC_API_KEY / OPENAI_API_KEY / BLOB_READ_WRITE_TOKEN / CRON_SECRET / ADMIN_ALLOWED_EMAILS / ADMIN_BASIC_AUTH_USER / ADMIN_BASIC_AUTH_PASSWORD）
  - `apps/web/.env.local.example` 作成（ローカル開発者向け）
  - `packages/db/drizzle.config.ts` の DATABASE_URL 読み取り設定
  - `packages/db/src/client.ts` で環境別 DB クライアント初期化（DATABASE_URL 必須、未設定で fail fast）
  - Vercel ビルド設定（root directory = `apps/web`、Build Command、Install Command）
  - `vercel.json` 作成: Cron 定義（`/api/cron/audio-purge` を `0 18 * * *` UTC = 03:00 JST 毎日実行）
  - 環境変数を Vercel に登録する手順の文書化
  - drizzle-kit push（dev branch）と migrate（production branch）の運用手順
  - `.github/workflows/ci.yml`（最小限：型チェック + lint + audit）
...
- **Out**:
  ...
  - Cloudflare R2（bulr Stage 1 では Vercel Blob を使う、R2 は Stage 2 で移行検討）
  ...
  - `/api/cron/audio-purge` のロジック実装（assessment-engine spec で実装、ここではルート定義のみ予約）
...
- Vercel Blob ストア定義 + トークン管理
- Vercel Cron 定義（`vercel.json`）
- CI 最小設定（GitHub Actions: typecheck + lint + audit）
...
- 音声削除 Cron のロジック実装（assessment-engine spec、ここでは vercel.json の Cron スケジュールのみ）
...
- **Downstream**:
  ...
  - `assessment-engine`（Anthropic / OpenAI API キー、Vercel Blob トークン、CRON_SECRET、DATABASE_URL を使う、Vercel Cron `/api/cron/audio-purge` ルートを実装）
  - `admin-review-panel`（Basic 認証用環境変数を使う、Vercel Blob からの音声取得サーバーサイドのみ）
...
- **Adjacent**:
  ...
  - `assessment-engine`: vercel.json の Cron スケジュールが共有契約
...
- **`tech.md` 準拠**:
  ...
  - Vercel Blob 無料枠（1GB/月、Stage 1 規模なら無料枠内）
  - Vercel Cron（Hobby プラン: 1 日 2 回実行制限あり、Stage 1 は 1 日 1 回で十分）
...
- **`security.md` 準拠**:
  ...
  - `CRON_SECRET` は Vercel Cron が自動付与する値を使う、手動呼び出し拒否
...
- **手動セットアップが残る**: Vercel / Neon / Resend / OpenAI / Anthropic のアカウント作成と初期設定は人間が手動実施する。spec ではその手順を文書化し、設定ファイル + .env.example で再現性を保つ
- **Vercel Cron 時刻**: 03:00 JST 固定（日本ユーザーが少ない時間帯）、UTC では 18:00、cron 表現は `0 18 * * *`
