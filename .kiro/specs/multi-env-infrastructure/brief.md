...
- **In**:
  ...
  - `.env.example` 作成（DATABASE_URL / BETTER_AUTH_SECRET / BETTER_AUTH_URL / RESEND_API_KEY / NEXT_PUBLIC_APP_URL / ANTHROPIC_API_KEY / OPENAI_API_KEY / BLOB_READ_WRITE_TOKEN / CRON_SECRET / ADMIN_ALLOWED_EMAILS）
...
- **Downstream**:
  ...
  - `admin-review-panel`（`ADMIN_ALLOWED_EMAILS` を使う、Vercel Blob からの音声取得サーバーサイドのみ）
