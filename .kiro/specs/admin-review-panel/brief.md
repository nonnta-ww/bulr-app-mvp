...
- `authentication` で `requireAdmin`、`adminAction`、`/admin/login` ログインページ、smoke test ページ `/admin/_health/`（assessment-engine spec で削除済み）あり
...
- 創業者は `/admin/login` 経由で Magic Link サインイン + 許可メール検査で `/admin/sessions` にアクセス可能
...
- **認証**: `authentication` spec で確立した Server Component の `requireAdmin`（ADMIN_ALLOWED_EMAILS 許可メール検査）を使う
...
- **Upstream**:
  ...
  - `authentication`（`requireAdmin`、`adminAction`、`/admin/login`、smoke test ページ）
