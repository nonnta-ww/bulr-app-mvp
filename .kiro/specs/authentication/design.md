...
**Purpose**: 本機能は、bulr Stage 1 MVP プロトタイプ（AI 面接アシスタント型）における **2 種類の認証境界** を確立する。Better Auth 1.6.x + Magic Link + Resend で **面接官（interviewer = user）** の Magic Link サインインを提供し、`ADMIN_ALLOWED_EMAILS` 許可メール検査で **創業者（admin）** の管理画面アクセスを制御する。多層防御（CVE-2025-29927 教訓）に基づき、`proxy.ts` は UX リダイレクトのみを担当し、認可は Server Component / Server Action / API Route の各レイヤーで独立にチェックする。
...
- proxy.ts（`apps/web/proxy.ts`）で UX リダイレクトを実装、JSDoc に CVE-2025-29927 教訓を明記
...
- `apps/web/lib/auth/schemas.ts`（Zod スキーマ集約: メール、面接官プロファイル）
...
- `apps/web/proxy.ts`（旧 middleware.ts、UX リダイレクトのみ、JSDoc に CVE-2025-29927 教訓明記）
...
- **制約**: `packages/auth` を作らない（Stage 1）。新規環境変数は追加しない（`multi-env-infrastructure` で確定済みの 10 変数のみ使用）。テストフレームワーク（Vitest 等）は導入しない（手動スモークテストで完了確認）。
...
graph TB
    subgraph Browser[面接官 / 創業者の Browser]
        ...
        AdminLoginPage[/admin/login<br/>Magic Link サインイン案内]
    ...
    subgraph ProxyLayer[Layer 1: proxy.ts UX のみ]
        Proxy[apps/web/proxy.ts<br/>/interviews/* Cookie 存在チェック → /sign-in リダイレクト<br/>/admin/* は対象外<br/>JSDoc: CVE-2025-29927 教訓]
    end
...
| Validation         | Zod 4.x（既設）                                | メール / 面接官プロファイル入力検証                  | `apps/web/lib/auth/schemas.ts` に集約                                    |
...
| Proxy              | Next.js 16 proxy.ts（旧 middleware.ts）        | UX リダイレクトのみ                                  | `apps/web/proxy.ts`、JSDoc に CVE-2025-29927 教訓明記                    |
...
├── apps/
│   └── web/
│       ├── proxy.ts                                    # 新規: UX リダイレクトのみ + CVE-2025-29927 JSDoc
...
sequenceDiagram
    ...
    U->>H: GET /admin/_health (proxy.ts は /admin/* を対象外)
    H->>G: requireAdmin()
    G->>Srv: getSession()
...
| 4.1         | requireAdmin() 許可メール検査                         | Guards                                              | guards.ts                                   | Admin      |
| 4.2         | /admin/\* で requireAdmin 必須                        | Guards + 全 admin Server Component                  | guards.ts                                   | Admin      |
| 4.3         | /admin/login UI                                       | AdminLoginPage                                      | apps/web/app/admin/login/page.tsx           | Admin      |
| 4.4         | ADMIN_ALLOWED_EMAILS 未設定で fail secure             | Guards                                              | guards.ts                                   | —          |
| 4.5         | /admin/\_health smoke test                            | AdminHealthPage                                     | apps/web/app/admin/\_health/page.tsx        | Admin      |
...
| 6.4         | matcher 設定 (/interviews/\* のみ)                    | Proxy                                               | proxy.ts config.matcher                     | —          |
| 6.5         | bypass 攻撃でも Layer 2 で防御                        | Guards + 全 Server Component                        | guards.ts                                   | —          |
| 6.6         | proxy.ts は cookie 存在のみ確認                       | Proxy                                               | proxy.ts                                    | —          |
...
| 9.2         | display_name Zod                                      | AuthSchemas                                         | schemas.ts                                  | —          |
| 9.3         | role_in_org Zod                                       | AuthSchemas                                         | schemas.ts                                  | —          |
| 9.4         | years_of_experience Zod                               | AuthSchemas                                         | schemas.ts                                  | —          |
| 9.5         | Server Action ラッパーで parse                        | SafeAction                                          | safe-action.ts                              | —          |
| 9.6         | スキーマ集約                                          | AuthSchemas                                         | apps/web/lib/auth/schemas.ts                | —          |
...
| 11.6        | Magic Link サインイン案内                             | AdminLoginPage                                      | page.tsx                                    | Admin      |
...
| AdminHealthPage     | apps/web/app/admin | smoke test ページ                                            | 4.5, 10.1-10.8                                            | Guards (P0)                                                                                                                       | State          |
| Proxy               | apps/web           | UX リダイレクトのみ                                          | 3.7, 6.1-6.6                                              | (Next.js proxy.ts API)                                                                                                            | Service        |
...
| Field        | Detail                       |
| ------------ | ---------------------------- |
| Intent       | 管理画面ログイン案内ページ   |
| Requirements | 4.3, 11.5, 11.6, 11.7        |

**Responsibilities & Constraints**

- 「管理者メールアドレスで Magic Link サインインしてください」表示
- `/sign-in?redirect=/admin/_health` へのリンク
- サインイン済み + 許可メールに含まれる場合は `/admin/_health` へ自動 redirect
...
| Field        | Detail                                                                   |
| ------------ | ------------------------------------------------------------------------ |
| Intent       | UX リダイレクトのみ、CVE-2025-29927 教訓を JSDoc に明記                  |
| Requirements | 3.7, 6.1-6.6                                                             |
...
- `config.matcher`:
  ...
  ```
- `/admin/*` は matcher 対象外。`/admin/_health/` や `/admin/sessions/*` の認可は Server Component の `requireAdmin()` が独立に行う
...
8. **`/admin/_health` 3 ケース**
   - (a) 未サインイン → `/sign-in` リダイレクト
   - (b) サインイン済み + メール非許可 → 「FORBIDDEN」表示
   - (c) サインイン済み + メール許可 → 「OK: admin authenticated」+ email 表示
   - Requirements: 4.1, 4.4, 4.5, 10.1-10.8

9. **多層防御（CVE-2025-29927 シミュレーション）**
   - proxy.ts を一時的に無効化（matcher を空に）して `/admin/_health` にアクセス → Server Component の `requireAdmin()` が独立に弾く（401 / 403 表示）
   - 検証後 proxy.ts を元に戻す
   - Requirements: 6.5, 4.2, 3.8
