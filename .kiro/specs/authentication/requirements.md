...
bulr Stage 1 MVP プロトタイプ（AI 面接アシスタント型）における **2 種類の認証境界** を確立するスペック。`monorepo-foundation` で構築された apps/web スケルトンと、`multi-env-infrastructure` で整備された環境変数規約（`BETTER_AUTH_SECRET` / `BETTER_AUTH_URL` / `RESEND_API_KEY` / `ADMIN_ALLOWED_EMAILS` / `ADMIN_BASIC_AUTH_USER` / `ADMIN_BASIC_AUTH_PASSWORD`）の上に、Better Auth 1.6.x + Magic Link で **面接官（interviewer = user）** 認証を実装し、Basic 認証 + 許可メール二重チェックで **創業者（admin）** 認証を実装する。
...
- **In scope**:
  ...
  - 管理画面ログイン UI（`apps/web/app/admin/login/page.tsx`、Basic 認証用ガイダンス）
  - `proxy.ts`（旧 middleware.ts、`apps/web/proxy.ts`）: 管理画面 Basic 認証チェック + 面接官 UX リダイレクト（`/interviews/*` 未認証時に `/sign-in` へ）。JSDoc に CVE-2025-29927 の教訓と「セキュリティ責任は持たない」旨を明記
  ...
  - Zod 入力検証: メール形式（Magic Link 送信時）、Basic 認証情報、面接官プロファイル入力（display_name、role_in_org? 等）
...
- **Adjacent expectations**:
  ...
  - `multi-env-infrastructure` で `.env.example` および Vercel に登録された `BETTER_AUTH_SECRET` / `BETTER_AUTH_URL` / `RESEND_API_KEY` / `ADMIN_ALLOWED_EMAILS` / `ADMIN_BASIC_AUTH_USER` / `ADMIN_BASIC_AUTH_PASSWORD` を本スペックが参照する。新規環境変数は追加しない
  ...
  - `proxy.ts` の Basic 認証チェックロジックは後続 `admin-review-panel` spec で `/admin/sessions/*` ページを保護する前提を提供する（本スペックでは `/admin/_health/` smoke test ページのみで動作確認）
...
### Requirement 4: 管理者認証（Basic 認証 + ADMIN_ALLOWED_EMAILS 二重チェック）

**Objective:** As a 創業者（admin）, I want `/admin/*` 配下が Basic 認証と許可メールリストの両方を通過した場合のみアクセス可能であること, so that 管理画面が単一の防御層に依存せず、Basic 認証の漏洩や設定ミスがあっても許可メール検査で防げる多層防御が機能する。

#### Acceptance Criteria

1. The proxy.ts shall リクエストパスが `/admin/*` で始まる場合、`Authorization` ヘッダーから Basic 認証情報を取得し、`ADMIN_BASIC_AUTH_USER` と `ADMIN_BASIC_AUTH_PASSWORD` 環境変数と照合する。
2. If `Authorization` ヘッダーが存在しない、または認証情報が不一致の場合、then the proxy.ts shall HTTP 401 と `WWW-Authenticate: Basic realm="bulr admin"` ヘッダーを返す。
3. The apps/web shall `apps/web/lib/guards.ts` に `requireAdmin()` 関数を持ち、Better Auth セッションを取得 → セッション未存在なら `AuthError('UNAUTHORIZED')` → セッションのユーザーメールが `ADMIN_ALLOWED_EMAILS`（CSV、`process.env.ADMIN_ALLOWED_EMAILS?.split(',').map(s => s.trim()) ?? []`）に含まれなければ `AuthError('FORBIDDEN')` を発生させる。
4. The Server Component / API Route shall `/admin/*` 配下のすべてのページとエンドポイントで `requireAdmin()` を呼び出し、proxy.ts の Basic 認証チェックに依存しない（多層防御）。
5. The apps/web shall `apps/web/app/admin/login/page.tsx` を持ち、Basic 認証ダイアログが表示される旨と、Basic 認証通過後に Magic Link でサインインする必要がある旨をユーザーに案内する。
6. The proxy.ts shall `/admin/*` 配下に対して Basic 認証チェック後にも、Better Auth セッションの有無を検証せず（UX 責務のみ）、セキュリティ判定は Server Component の `requireAdmin()` に委ねる。
7. When `ADMIN_ALLOWED_EMAILS` が未設定または空文字の場合、then the requireAdmin() shall 全ユーザーを拒否（fail secure）し、`AuthError('FORBIDDEN')` を発生させる。
8. The apps/web shall `apps/web/app/admin/_health/page.tsx` を一時設置し、`requireAdmin()` を呼び出す Server Component として、Basic 認証 + Magic Link サインイン + 許可メール検査の 3 つすべてを通過した場合のみ「OK」と表示する（admin-review-panel spec で `/admin/sessions` を実装した時点で削除予定）。
...
**Objective:** As a 面接官および創業者, I want 未認証で `/interviews/*` にアクセスしたら自動的に `/sign-in` にリダイレクトされ、`/admin/*` で Basic 認証ダイアログが表示されること, so that 適切な認証フローに自然に誘導される。同時に、proxy.ts の脆弱性（CVE-2025-29927 のような bypass 攻撃）があっても、各レイヤーの独立チェックで防御が破られない多層構造であること。
...
4. The proxy.ts shall リクエストパスが `/admin/*` で始まる場合、`Authorization` ヘッダーから Basic 認証情報を抽出し、`ADMIN_BASIC_AUTH_USER` / `ADMIN_BASIC_AUTH_PASSWORD` と照合する。
5. If `/admin/*` への Basic 認証が失敗、then the proxy.ts shall HTTP 401 と `WWW-Authenticate: Basic realm="bulr admin"` を返す。
6. The proxy.ts shall `/admin/_health` 等の smoke test ルートを `/admin/*` の Basic 認証チェック対象に含め、特別扱いしない。
7. The proxy.ts shall `config.matcher` で `/interviews/:path*`、`/admin/:path*` を対象とし、`/api/auth/*`、`/_next/*`、静的ファイル等は対象外とする。
8. When 攻撃者が proxy.ts の bypass を試みた場合（例: 特定ヘッダーで matcher を回避する CVE-2025-29927 類似攻撃）、then the Server Component / Server Action / API Route shall `requireUser()` / `requireAdmin()` で独立に認可を判定し、データを露出しない。
9. The proxy.ts shall Better Auth のセッション検証ロジック自体は実行せず、Cookie の存在のみを確認する（実 session validation は Server Component で `requireUser()` が行う、UX 責務のみ）。
...
### Requirement 9: Zod 入力検証（メール形式 / Basic 認証情報 / 面接官プロファイル入力）

**Objective:** As a セキュリティ責任者, I want すべての外部入力（Magic Link 送信時のメール、Basic 認証情報、面接官プロファイル入力）が Zod スキーマで検証されること, so that 不正なフォーマットの入力でアプリケーションが落ちたり、SQL インジェクションや XSS 攻撃の足がかりにされたりしない。
...
2. The proxy.ts の Basic 認証ロジック shall `Authorization` ヘッダーの形式（`Basic <base64>`）と base64 デコード後の `user:password` 形式を検証してから環境変数と照合する（不正形式の場合は HTTP 401）。
3. The 面接官プロファイル入力 shall `display_name` を Zod の `z.string().trim().min(1).max(100)` で検証する。
4. The 面接官プロファイル入力 shall `role_in_org` を Zod の `z.string().trim().max(100).optional()` で検証する。
5. The 面接官プロファイル入力 shall `years_of_experience` を Zod の `z.number().int().min(0).max(60).optional()` で検証する。
6. The Server Action ラッパー（`authedAction` / `adminAction`）shall すべての入力を Zod schema で `parse()` してから handler を呼び、`ZodError` を捕捉して Server Action の戻り値として `{ ok: false, error: { code: 'INVALID_INPUT', message } }` を返す。
7. The Zod schema 定義 shall `apps/web/lib/auth/schemas.ts`（または同等）に集約され、複数箇所から再利用可能な形でエクスポートされる。

### Requirement 10: smoke test ページ（一時設置、admin-review-panel で削除）

**Objective:** As a 創業者（本スペック完了確認担当）, I want `/admin/_health/` ページにアクセスして、Basic 認証 + Magic Link サインイン + 許可メール検査の 3 つすべてが正しく動作することを目視確認できること, so that 本スペック完了時点で実認証パスが通ることを検証でき、後続 `admin-review-panel` spec の `/admin/sessions` 実装に進める。
...
6. When 創業者が Basic 認証通過 + Magic Link サインイン済み + メールが ADMIN_ALLOWED_EMAILS に含まれる状態で `/admin/_health/` にアクセスしたとき、the apps/web shall 「OK: admin authenticated」と該当メールを表示する。
7. When Basic 認証通過後・Magic Link サインインしていない状態で `/admin/_health/` にアクセスしたとき、the apps/web shall `/sign-in` にリダイレクトする。
8. When Basic 認証通過 + Magic Link サインイン済みだがメールが ADMIN_ALLOWED_EMAILS に含まれない状態で `/admin/_health/` にアクセスしたとき、the apps/web shall HTTP 403 相当の FORBIDDEN 表示を行う。
...
5. The apps/web shall `apps/web/app/admin/login/page.tsx` を持ち、Basic 認証ダイアログが表示される旨と、Basic 認証通過後に Magic Link でサインインする必要がある旨をユーザーに案内する。
6. The /admin/login ページ shall Basic 認証通過後に表示され、「Basic 認証通過 OK。次に管理者メールアドレスで Magic Link サインインしてください」のメッセージとサインインリンク（`/sign-in?redirect=/admin/_health`）を表示する。
