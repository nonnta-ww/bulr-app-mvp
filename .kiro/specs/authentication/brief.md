...
- **管理画面**: `proxy.ts` 内で Authorization ヘッダーをチェック（`ADMIN_BASIC_AUTH_USER` / `ADMIN_BASIC_AUTH_PASSWORD` と照合）→ Server Component で `requireAdmin()` が `ADMIN_ALLOWED_EMAILS` と Better Auth セッションを照合する二段構成
...
- **In**:
  ...
  - `proxy.ts`（旧 middleware.ts）: 管理画面 Basic 認証チェック + 面接官ガード（未認証の `/interviews/*` を `/sign-in` にリダイレクト）
...
- **Downstream**:
  ...
  - `admin-review-panel`（`requireAdmin` で管理画面ガード、Basic 認証チェック、smoke test ページの削除・置き換え）
...
- **`security.md` 準拠**:
  ...
  - Zod で全入力を検証（メール形式、Basic 認証情報、面接官プロファイル入力）
- **`structure.md` 準拠**:
