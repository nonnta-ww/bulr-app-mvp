...
- [x] 1. Foundation: スキーマ enum 拡張とシード共通ランナーの受け入れ拡張
- [x] 1.1 `survey_kind` enum への値追加と migration
...
- [x] 1.2 seed 共通ランナーの kind union 拡張
...
- [x] 2. Core: 志向アンケート seed データ投入
- [x] 2.1 志向アンケート seed データの作成
...
- [x] 3. Core: 志向スコアリング純関数（app ローカル）
- [x] 3.1 (P) カテゴリ対応表と回答マッピング
...
- [x] 4. Core: DB query（survey id 解決・本人回答取得）
- [x] 4.1 (P) survey id 解決 query
...
- [x] 5. Integration: class-diagnosis への配線
- [x] 5.1 page.tsx での取得・ライブスコアリング
  ...
  - _Depends: 3.2, 4.2_
- [x] 5.2 ClassDiagnosisView・ClassCard・SharePanel への props 中継
...
- [x] 6. Validation: UI 回帰・統合的な弁別力の検証
- [x] 6.1 (P) ClassCard/SharePanel のコンポーネントテスト更新
