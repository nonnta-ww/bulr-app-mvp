/**
 * Business 向け Magic Link メールテンプレート
 * 純関数。副作用なし。受信者の個人情報（メールアドレス等）を本文に含まない。
 *
 * packages/auth/src/email/templates/magic-link.ts の企業向け内容を移設したもの。
 * 削除元 (magic-link.ts) は task 2.3 で対応する。
 *
 * Requirements: 1.6, 2.2, 8.2
 */

/**
 * Business 向け Magic Link メールの件名・HTML・プレーンテキストを生成して返す純関数。
 *
 * @param url - Magic Link の URL（Better Auth が生成したトークン付き URL）
 * @returns { subject, html, text }
 */
export function renderBusinessMagicLinkEmail({ url }: { url: string }): {
  subject: string;
  html: string;
  text: string;
} {
