/**
 * 管理画面 企業一覧ページ（apps/admin: /companies）
 *
 * Server Component。Layer 2 多層防御として requireAdmin() を先頭で呼び出す。
 * searchParams から search / page / isActive を取得し、getCompaniesForAdmin で
 * データを取得して SearchFilter + テーブルを描画する。
 * ページ下部に CreateCompanyForm を配置して新規企業を作成できる。
 *
 * Requirements: 2.1, 2.2, 2.3, 2.4, 6.1, 6.6
 * Boundary: CompanyListPage (this file only)
 * Depends: 3.1 ✓ (getCompaniesForAdmin), 8.1 ✓ (createCompany)
 */

import Link from 'next/link';
import { notFound, redirect } from 'next/navigation';

import { CreateCompanyForm } from '@/app/_components/create-company-form';
import { SearchFilter } from '@/app/_components/search-filter';
import { AuthError, requireAdmin } from '@bulr/auth/server';
import { getCompaniesForAdmin } from '@bulr/db/queries/admin';
...
export default async function CompanyListPage({ searchParams }: PageProps) {
  ...
  // DBクエリ
  const { items, total } = await getCompaniesForAdmin({
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <h1 className="mb-6 text-2xl font-bold text-gray-900">企業一覧</h1>

      {/* フィルタ行 */}
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <SearchFilter placeholder="企業名で検索" paramKey="search" />

        {/* 有効/無効フィルタ（GET フォーム） */}
        <form method="GET" action="/companies" className="flex items-center gap-2">
      ...
      {/* テーブル */}
      {items.length === 0 ? (
        <p className="py-8 text-center text-sm text-gray-500">企業がありません</p>
      ) : (
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-gray-600">募集数</th>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {items.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right text-gray-700">{item.openingCount}</td>
