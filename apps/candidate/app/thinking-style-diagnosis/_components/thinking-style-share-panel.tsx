...
/**
 * thinking-style-share-panel.tsx — 思考スタイル診断の共有パネル（task 4.3 / R4.1/4.2/4.3）
 *
 * 思考スタイルアーキタイプ（Archetype）から、アーキタイプ名（＋任意で shortLabel）のみで
 * 構成した共有テキストを生成し、プレビューとコピー/共有ボタンを表示する。
 *
 * - toThinkingStyleShareText は純関数。archetype.name（と任意で shortLabel）・固定ブラーブ／
 ...
 * - 共有テキストは現在の診断結果（archetype）からのみ導出し、追加の永続化は行わない（R4.3）。
 * - このコンポーネントは親が結果表示できるときのみマウントする。ここでは有効な
 ...
 *
 * Boundary: ThinkingStyleSharePanel, toThinkingStyleShareText
...
const SHARE_HASHTAG = '#思考スタイル診断';
const SHARE_BLURB = 'あなたの思考スタイルは？';
 ...
 */
export function toThinkingStyleShareText(archetype: ShareArchetype): string {
  const lines = [`私の思考スタイルは「${archetype.name}」でした。`, SHARE_BLURB, SHARE_HASHTAG];
  return lines.join('\n');
}

interface ThinkingStyleSharePanelProps {
  archetype: Archetype;
}
 ...
 */
export function ThinkingStyleSharePanel({ archetype }: ThinkingStyleSharePanelProps) {
  const [copied, setCopied] = useState(false);
  const shareText = toThinkingStyleShareText(archetype);
  ...
  return (
    <section
      ...
      aria-label="診断結果の共有"
      data-testid="thinking-style-share-panel"
    >
      ...
      <pre
        ...
        data-testid="thinking-style-share-preview"
      >
        {shareText}
      </pre>
      ...
      <button
        ...
        data-testid="thinking-style-share-button"
      >
      ...
      {copied ? (
        <p
          ...
          role="status"
          data-testid="thinking-style-share-copied"
        >
          コピーしました
        </p>
      ) : null}
