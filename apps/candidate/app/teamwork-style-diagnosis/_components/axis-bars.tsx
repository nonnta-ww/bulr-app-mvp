/**
 * axis-bars.tsx — チームワーク・スタイル4軸のバイポーラ可視化（task 5.1 / R4.4, R9.2）
 *
 * 各軸を「第1極 ⇔ 第2極」のトラックとして描き、AxisReading.score の寄りをトラック上の
 * マーカー位置（0..100 → 0%..100%）だけで示す。**数値ラベルは一切描画しない**（position-only,
 * R4.4/R9.2）。未回答（determined=false）の軸は淡色トラック＋「未回答」表示にし、確信的なマーカーは
 * 置かず中立状態として区別する。
 *
 * presentational / summary-only。フック不使用のため 'use client' は付けない。
 * バー表示順は AXES canonical order（決定論）。thinking-style の AxisBars と同型（app-local 複製）。
 *
 * Boundary: axis-bars
 * Requirements: 4.4, 9.2
 */

import { AXES, AXIS_LABELS, type TeamworkAxis } from '../../_lib/teamwork-style/axes';
import type { AxisReading } from '../../_lib/teamwork-style/score';
 ...
 * score はマーカー位置にのみ使用し、数値としては一切描画しない（R4.4/R9.2）。
 */
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between text-sm font-medium">
              <span className={determined ? 'text-gray-900' : 'text-gray-400'}>
                {label.first}
              </span>
              <span className={determined ? 'text-gray-900' : 'text-gray-400'}>
                {label.second}
