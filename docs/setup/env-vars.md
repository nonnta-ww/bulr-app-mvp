...
| `EMAIL_FROM_ADDRESS` | メール送信元アドレス（**検証済みドメイン**のアドレス） | サーバー専用 | Production / Preview（3プロジェクト全て）。未設定だとテストドメインになり所有者以外への送信が 500 | 例 `bulr <noreply@bulr.net>`。詳細は [`./resend.md` §7](./resend.md#7-本番ドメイン検証実ユーザー解放に必須) |
