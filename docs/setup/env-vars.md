...
| `RESEND_API_KEY` | メール送信 API キー | サーバー専用 | Production / Preview / Development | Resend ダッシュボード → API Keys |
...
| `ADMIN_BASIC_AUTH_USER` | 管理画面 Basic 認証ユーザー名 | サーバー専用 | Production / Preview / Development | 手動設定 |
| `ADMIN_BASIC_AUTH_PASSWORD` | 管理画面 Basic 認証パスワード | サーバー専用 | Production / Preview / Development | `openssl rand -base64 16` で生成 |
