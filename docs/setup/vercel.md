...
- **Neon 両ブランチで `pnpm --filter @bulr/db migrate` 実行済み** — Better Auth の `verification` / `user` / `session` / `account` テーブル等が無いと Production magic link が 500 になる（[`drizzle-kit.md`](./drizzle-kit.md) 参照）
- Resend で **送信ドメイン `bulr.net` を検証済み** — 未検証だと API キー所有者宛以外の magic link が 500 になる（[`resend.md`](./resend.md) 参照）
...
## Step 1.5: Deploy Flow（Production / Preview の振り分け）

Vercel は **Git push の宛先ブランチ**で Production / Preview を自動振り分けする。設定は Step 1.1 の `Production Branch: main` がベース。

| 契機 | デプロイ種別 | URL | 使われる env |
|---|---|---|---|
| `main` への push（マージ含む） | **Production** | カスタムドメイン (`bulr.net` / `bz.bulr.net` / `admin.bulr.net`) + Vercel デフォルト URL | Production env |
| 他のブランチへの push（`develop` / `feature/*` / `fix/*` 等、名前は何でも可） | **Preview** | ユニーク URL `bulr-mvp-business-<7文字hash>-nonnta-wws-projects.vercel.app` | Preview env |
| Pull Request の作成 / 更新 | **Preview** | 上と同様（PR コメントに 3 つの Preview URL が貼られる） | Preview env |

### 重要な性質

- **Preview URL は push のたびにユニーク**（同じブランチでも commit ごとに別 URL）
- 同じブランチの**最新を指す固定 URL** が欲しい場合は Vercel Project → Settings → Domains で **Branch Alias**（例: `develop.bulr.net` を `develop` ブランチに付与）
- env は Vercel Project → Settings → Environment Variables で Production / Preview / Development を切替えて登録（Step 2 参照）
- Preview deploy は **Deployment Protection (Vercel SSO)** が Pro plan の default で有効。チームメンバーがブラウザで Vercel にログインしていないと 401 になるため、未ログイン状態の curl では Preview URL アクセス不可

### 推奨ブランチ運用

- **2 段階フロー** が標準: feature branch → PR → `develop` → PR → `main`
  - feature や fix の検証は Preview URL（個別）
  - `develop` を staging エイリアスに使い、統合 Preview として運用
- **シンプルなフロー** で済ませる場合: feature branch → PR → `main` 直接
  - Wave 1 はこのフローで完了
...
| `EMAIL_FROM_ADDRESS` | `*` | `*` | `*` | 例: `bulr <noreply@bulr.net>`。Resend で `bulr.net` 検証済みであること（[`resend.md`](./resend.md) 参照）。未設定時は `onboarding@resend.dev` にフォールバックするが、テストモードでは API キー所有者宛のみ送信可 |
...
### Magic Link 送信時に 500: `relation "verification" does not exist`

Neon Production ブランチに drizzle-kit migration が未実行。Better Auth が必要とする `verification` / `user` / `session` / `account` テーブルが無い。ローカルから:

```bash
DIRECT_URL="<Neon Production direct (unpooled) URL>" \
  pnpm --filter @bulr/db migrate
```

Preview もエラーになる場合は Neon Dev ブランチでも同様に実行。

### Magic Link 送信時に 500: `You can only send testing emails to your own email address`

Resend が「テストモード」で動作している。原因と対処:

1. **Resend のドメイン未検証** → resend.com/domains で `bulr.net` を追加して DNS 検証
2. **`EMAIL_FROM_ADDRESS` 未設定** → Vercel に `EMAIL_FROM_ADDRESS=bulr <noreply@bulr.net>`（検証済みドメイン）を登録
3. 上記の暫定回避としては、`API キー所有者のメアド宛`で動作確認するのが可（テストモード制限内）
